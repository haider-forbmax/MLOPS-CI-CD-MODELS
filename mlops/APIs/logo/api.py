...
app = FastAPI(title="YOLOv11n Logo Detection API", version="1.0.0")
...
async def detect_Logos(request: DetectionRequest, authorization: str = Header(None)):
    """Detect Logos in image using YOLO11 model"""

    ## overwrite model name from request with config value to ensure correct model is used
    ...
    if not (0 <= request.parameters.confidence <= 1):
        logger.error(f"Invalid confidence value: {request.parameters.confidence}")
        return JSONResponse(
            status_code=400,
            content={
    ...
    if not (0 <= request.parameters.nms_threshold <= 1):
        logger.error(f"Invalid nms_threshold value: {request.parameters.nms_threshold}")
        return JSONResponse(
            status_code=400,
            content={
    ...
    try:
        ...
        for detection in final_detections:
            ...
            if request.response_format.object_images:
                ...
                if object_image_b64 is None:
                    logger.warning(f"Failed to crop Logo {class_name} with invalid bounding box [{x1}, {y1}, {x2}, {y2}]")
        ...
        logger.debug(f"Detection completed: {len(detection_objects)} Logos found in {total_time:.2f}ms")
        
        response = DetectionResponse(
            id=f"detect_yolo11_{uuid.uuid4().hex[:8]}",
    ...
    try:
        ...
        if overall_status == "unhealthy":
            ...
            return JSONResponse(
                status_code=503,
                content={
                    "message": "Service is currently unhealthy. Please try again later.",
...
async def root():
    ...
    return {
        "service": "YOLO11 Logo Detection API",
        "version": "1.0.0",
        "description": "Logo detection API using YOLO11 models with Triton Inference Server",
        "endpoints": {
            "detect": "/v1/detect",
            "health": "/health",
            "classes": "/classes",
