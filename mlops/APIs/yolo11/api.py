...
async def detect_objects(request: DetectionRequest, authorization: str = Header(None)):
    """Detect objects in image using YOLO11 model"""
    
    ## overwrite request model with config value to ensure correct model is used
    ...
    # Get model-specific classes from Triton model config metadata.
    ...
    try:
        ...
        for detection in final_detections:
            ...
            class_name = model_classes.get(str(int(class_id)), f"class_{int(class_id)}")

            # Calculate bounding box coordinates (Standard corner format)
            x1 = max(0, int(x - w / 2))
            y1 = max(0, int(y - h / 2))
            ...
            if request.response_format.object_images:
                try:
                    ...
                    logger.warning(f"Error cropping object {class_name}: {str(e)}")
        ...
        if request.response_format.labeled_image:
            try:
                ...
                logger.error(f"Failed to create labeled image: {str(e)}")
        ...
        logger.exception("Unexpected internal error during detection")
        return JSONResponse(
            status_code=500,
            content={
...
async def get_all_classes():
    """Get class mappings from Triton model config metadata"""
    try:
        triton_client = detector.get_triton_client()
        try:
...
async def root():
    ...
    try:
        return {
            "service": "YOLO11 Object Detection API",
            "version": "1.0.0",
            "description": "Object detection API using YOLO11 models with Triton Inference Server",
            "endpoints": {
                "detect": "/v1/detect",
                "health": "/health",
                "classes": "/classes",
        ...
        logger.error(f"Failed to load root endpoint: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={
